...
angular.module('productApp', [
    ...
    function ($scope, Fabric, FabricConstants, Keypress, $http, $timeout, $mdDialog, $mdToast, Upload) {
        ...
        var SETTINGS_INIT_URL = "http://localhost/artaholics_dev/assets/design/inc/json/settings.json";
        ...
        $scope.onFileSelectUrl = function(src=''){
            var src = $("#srcimg").attr('src');
            if(src !== null) {
                if ($scope.fabric.checkBackgroundImage())
                {                   
                    $scope.fabric.addImage(src);
                    $scope.objectLayers = [];
                    $scope.objectLayers = $scope.fabric.canvasLayers();
                }else{
                    _this.showNotification($scope.NOTIFICATION_MESSAGES.CANVAS_EMPTY, true);
        ...
        this.initProducts = function () {
            ...
            $http({
                method: 'get',
                url: "http://localhost/artaholics_dev/Design/get_costomised_products/",
                dataType : 'json',
                headers: {'Content-Type': 'application/json'}
            }).success(function (data, status, headers, config) {
